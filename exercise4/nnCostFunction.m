...
labels = (1:num_labels);   % 1 x num_label(10)

% a1 = X;   % 5000 x 400
% a2 = sigmoid([ones(m, 1) a1] * Theta1');   % 5000 x 25
% a3 = sigmoid([ones(m, 1) a2] * Theta2');   % 5000 x 10
...
for i=1:m
  ...
  % Output layer
  h_t = a3';    % 10 x 1
  y_t = labels == y(i);   % 1 x 10

  % Cost function
  J = J + ((-y_t * log(h_t) - (1-y_t) * log(1-h_t)));

  % Error at every units
  d3 = (a3 - y_t)';    % 10 x 1
...
% Average regularized cost function
temp = [Theta1(:,(2:end))(:); Theta2(:,(2:end))(:)];
J = (1/m) * J + (lambda/(2*m)) * (temp' * temp);
